...
(function(){
    ...
    UM.registerWidget( widgetName,{
        ...
        tpl: "<link rel=\"stylesheet\" type=\"text/css\" href=\"<%=video_url%>video.css\" />" +
            ...
            "<div id=\"eduiVideoTab\">" +
            "<p style=\"float: right\">暂时只支持优酷和腾讯的视频播放页面链接</p>" +
        ...
        createPreviewVideo: function(url){
            ...
            $("#eduiVideoPreview", me.$widget)[0].innerHTML = ''
                + '<iframe height="280" width="420" src="'
                + conUrl
        ...
        convert_url: function(url){
            ...
            var matches = url.match(/youtu.be\/(\w+)$/) ||
                ...
                youku = url.match(/youku\.com\/v_show\/id_([^\.]+)/),
                youkuPlay = /player\.youku\.com/ig.test(url);

            var qq = url.match(/qq\.com\/[^?]+\?vid=(\w+)/);

            /*var reverseUrl = url.split('').reverse().join('');
            var tudou = reverseUrl.match(/lmth\.([^\/]+)\/.*\/moc\.uodut/);*/
            ...
            if(youkuPlay){
                ...
                url = "http://player.youku.com/embed/"+youku[1];
            }
            else if (qq) {
                url = "http://v.qq.com/iframe/player.html?vid=" + qq[1] + '&tiny=0&auto=0';
            }
            else {
