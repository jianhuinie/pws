...
//                }
    ...
    var Upload = {
        ...
        //     '<input style=\"filter: alpha(opacity=0);\" class="edui-image-file" type="file" hidefocus name="attachment" accept="image/gif,image/jpeg,image/png,image/jpg,image/bmp"/>' +
        //     '</form>' +
        ...
        init: function (editor, $w) {
            ...
            me.uploadTpl = ''
                        ...
                        +      '<span class="edui-image-icon"></span>'
                        +      '<form class="edui-image-form" method="post" enctype="multipart/form-data" target="up">'
                        +          '<input style=\"filter: alpha(opacity=0);\" class="edui-image-file" type="file" hidefocus name="' + me.editor.options.imageFieldName + '" accept="image/gif,image/jpeg,image/png,image/jpg,image/bmp"/>'
                        +      '</form>'
                        +  '</div>';
        ...
        uploadComplete: function(r){
            ...
            try{
                var json;
                // 如果设置了这个标志位  特殊处理
                if (me.editor.options.isFormatImageHack) {
                    json = JSON.parse(r);
                    json.url = json.data.url;
                    json.state = json.code === 0 ? 'SUCCESS' : 'FAILED';
                } else {
                    json = eval('('+r+')');
                }
        ...
        submit: function (callback) {
            ...
            $(me.dialog).delegate( ".edui-image-file", "change", function ( e ) {
                ...
                $('<iframe name="up"  style="display: none"></iframe>').insertBefore(me.dialog).on('load', function(){
                    var r;
                    if (me.editor.options.isFormatImageHack) {
                        r = this.contentWindow.document.getElementsByTagName('pre')[0].innerHTML;
                    }
                    else {
                        r = this.contentWindow.document.body.innerHTML;
                    }
                    if(r == '')return;
                    me.uploadComplete(r);
                    $(this).unbind('load');
