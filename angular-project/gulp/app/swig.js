...
var navs = [
    ...
    },
    {
        text: '校区管理',
        type: 'branches',
        ngHref: '/main?tick={{orgInfo.number}}',
        shown: {
            'downloadThird': 'IS_BRANCHES_MANAGER',
            'mapp': 'IS_BRANCHES_MANAGER',
            'signup': 'IS_BRANCHES_MANAGER',
            'advisory': 'IS_BRANCHES_MANAGER',
            'main': 'IS_BRANCHES_MANAGER',
            'vip_center': 'IS_BRANCHES_MANAGER',
            'arrange': 'IS_BRANCHES_MANAGER'
        }
    },
    {
        ngHref: '{{ arrangeUrl }}',
        ...
        shown: {
            ...
            'main': 'AUTH_O["arrange"]',
            'vip_center': 'AUTH_O["arrange"]'
        }
    },
    {
        ngHref: '/vip_center.html?tick={{ orgInfo.number }}',
        text: '会员中心',
        type: 'vip_center',
        shown: {
        ...
        icon: 'http://img.gsxservice.com/12873635_igycchmk.png'
    },
    {
    ...
    },
    {
        ...
        shown: {
            'downloadThird': 'true',
            'mapp': 'AUTH_O["advisory"]',
            'signup': 'true',
            'advisory': 'true',
            'main': 'true',
            'vip_center': 'AUTH_O["advisory"]'
...
var downloadThirdOptions = {
    data: {
        navs: navs,
        pageType: 'downloadThird'
    },
    defaults: { varControls: ['<%=', '%>']}
};
...
gulp.task('app.swig', function () {
    ...
    gulp.src([
        ...
        .pipe(swig(mainOptions))
        .pipe(gulp.dest('app/'));

    gulp.src([
            'app/html/login.html'
        ])
        .pipe(swig(loginSwigOptions))
        .pipe(gulp.dest('app/'));

    gulp.src([
            'app/html/download-third.html'
        ])
        .pipe(swig(downloadThirdOptions))
        .pipe(gulp.dest('app/'));

    gulp.src([
            'app/html/mapp.html'
        ])
        .pipe(swig(appOptions))
        .pipe(gulp.dest('app/'));

    gulp.src([
            'app/html/register.html'
        ])
        .pipe(swig(registerOptions))
        .pipe(gulp.dest('app/'));

    gulp.src([
            'app/html/forget.html'
        ])
        .pipe(swig(forgetOptions))
        .pipe(gulp.dest('app/'));

    gulp.src([
            'app/html/classroom.html'
        ])
        .pipe(swig({
            data: {
                liveOrigin: 'http://test-live.gsxservice.com'
            }
        }))
        .pipe(gulp.dest('app/'));

    gulp.src([
            'app/html/weixin_exp.html'
        ])
        .pipe(swig({
            data: {
    ...
    // 报名
    gulp.src(['app/html/signup.html'])
        .pipe(swig(signupOptions))
        .pipe(gulp.dest('app/'));

    // 报名单收费凭证
    gulp.src(['app/html/signup-order.html'])
        .pipe(swig())
        .pipe(gulp.dest('app/'));

    // 预约咨询
    gulp.src(['app/html/advisory.html'])
        .pipe(swig(advisoryOptions))
        .pipe(gulp.dest('app/'));
