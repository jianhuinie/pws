...
<head>
	...
	<link rel="stylesheet/less" href="../css/myThesis.less" type="text/less">
	<link rel="stylesheet" type="text/css" href="../lib/bootstrap.css">
	<script type="text/javascript" src="../lib/jquery-1.12.2.min.js"></script>
	<script type="text/javascript" src="../common/showPics.js"></script>
	<script type="text/javascript" src="../js/myThesis.js"></script>
			...
			<div id="content">
				...
				<div id="content-right"> 
					...
					<div id="texts">
						<div id="left">
							<div id="tabs">
								<div id="released" class="active">已发表论文</div>
								<div id="prevote">待审核论文</div>
								<div id="voting">审核中论文</div>
								<div id="failed">未通过论文</div>
							</div>
							<div id="releasedThesis" class="thesis">
								<table class="table">
								 	<thead>
								 		<td>通过票数</td>
								 		<td>论文标题</td>
								 	...
								 	<?php
									    ...
									    $mysql=mysqli_connect("localhost","2012061516","940710","project") or die("数据库连接失败".mysql_error());
										$mysql->query("set names utf8");
										$sql="select * from thesis where username ='$username' and agreeVotes > 2 and voteCount = 5 ORDER BY releaseTime DESC";
										$query=$mysql->query($sql);
										while($row=mysqli_fetch_array($query)) {
											...
											$query1=$mysql->query($sql1);
											$row1=mysqli_fetch_array($query1);
											echo "<tr><td>".$row['agreeVotes']."</td><td><a href='thesisDetail.php?thesisNum=$thesisNum'>".$row['title']."</a></td>"."<td><a href='meetingDetail.php?meetingNum=$meetingNum'>".$row1['name']."</a></td><td>".$row['releaseTime']."</td></tr>";
										}		 
									?>
							 	</table>
							</div>
							<div id="prevoteThesis" class="thesis" style="display: none">
								<table class="table">
								 	<thead>
								 		<td>通过票数</td>
								 		<td>论文标题</td>
								 	...
								 	<?php
									    ...
									    $mysql=mysqli_connect("localhost","2012061516","940710","project") or die("数据库连接失败".mysql_error());
										$mysql->query("set names utf8");
										$sql="select * from thesis where username ='$username' and voteCount = 0 ORDER BY releaseTime DESC";
										$query=$mysql->query($sql);
										...
										while($row=mysqli_fetch_array($query)) {
											...
											$query1=$mysql->query($sql1);
											$row1=mysqli_fetch_array($query1);
											echo "<tr><td>".$row['agreeVotes']."</td><td><a href='thesisDetail.php?thesisNum=$thesisNum'>".$row['title']."</a></td>"."<td><a href='meetingDetail.php?meetingNum=$meetingNum'>".$row1['name']."</a></td><td>".$row['releaseTime']."</td><td><a href='updateThesis.php?thesisNum=$thesisNum'>编辑</a></td></tr>";
										}		 
									?>
							 	</table>
							</div>
							<div id="votingThesis" class="thesis" style="display: none">
								<table class="table">
								 	<thead>
								 		<td>通过票数</td>
								 		<td>论文标题</td>
								 	...
								 	<?php
									    ...
									    $mysql=mysqli_connect("localhost","2012061516","940710","project") or die("数据库连接失败".mysql_error());
										$mysql->query("set names utf8");
										$sql="select * from thesis where username ='$username' and voteCount > 0 and voteCount < 5  ORDER BY releaseTime DESC";
										$query=$mysql->query($sql);
										while($row=mysqli_fetch_array($query)) {
											$thesisNum = $row['number'];
											$meetingNum = $row['meetingNum'];
							...
							<div id="failedThesis" class="thesis" style="display: none">
								<table class="table">
								 	<thead>
								 		<td>通过票数</td>
								 		<td>论文标题</td>
								 	...
								 	<?php
									    ...
									    $mysql=mysqli_connect("localhost","2012061516","940710","project") or die("数据库连接失败".mysql_error());
										$mysql->query("set names utf8");
										$sql="select * from thesis where username ='$username' and voteCount =5  and agreeVotes < 3  ORDER BY releaseTime DESC";
										$query=$mysql->query($sql);
										while($row=mysqli_fetch_array($query)) {
											$thesisNum = $row['number'];
											$meetingNum = $row['meetingNum'];
						...
						<div id="right">
							...
							<?php
								$mysql=mysqli_connect("localhost","2012061516","940710","project") or die("数据库连接失败".mysql_error());
								$mysql->query("set names utf8");
								$sql="select * from thesis where voteCount > 2 ORDER BY releaseTime DESC LIMIT 15";
								$query=$mysql->query($sql);
								$count = 0;
								while($row=mysqli_fetch_array($query)) {
									$num = $row['number'];
