...
<head>
	...
	<link rel="stylesheet/less" href="../css/meetings.less" type="text/less">
	...
	<div id="container">
		<div id="box">
			...
			<div id="nav">
					 ...
					 <a href="meetings.php" class="active">会议预告</a> 
			...
			<div id="content">
				...
				<div id="content-right"> 
					<div id='searchForm'>
						<form method="post">	 
						    <input type="text" placeholder="搜索会议" name="searchMeeting" id="searchMeeting">
						    <input type="submit" value='Search' onclick="if (validateform()) { form.action='./searchMeetings.php' }"> 
						</form>
					</div> 
					<ul class="breadcrumb">
						<li>
					...
					<div id="texts">
						<div id="left">
							...
							<?php
							 	 	...
							 	 	$meetingPage = $_GET['meetingPage'];
								    $mysql=mysqli_connect("localhost","2012061516","940710","project") or die("数据库连接失败".mysql_error());
									$mysql->query("set names utf8");
									if ($meetingPage > 0) {
										$rangeDown = $meetingPage*8;
										//$rangeUp = $rangeDown+8;
										$sql="select * from meetings ORDER BY beginTime DESC LIMIT $rangeDown,8";
									}
									else {
										$sql="select * from meetings ORDER BY beginTime DESC LIMIT 0,8";
									}
									$query=$mysql->query($sql);
									while($row=mysqli_fetch_array($query)) {
										$num = $row['number'];
										echo "<tr><td>".$row['subject']."</td><td><a href='meetingDetail.php?meetingNum=$num'>".$row['name']."</a></td><td>".$row['beginTime']."</td><td>".$row['place']."</td><td>".$row['thesisDeadline']."</td></tr>";
									}		 
								?>
							</tbody>
						</table>
							<ul class="pager">
								<?php
									...
									$meetingPage = $_GET['meetingPage'];
									 $mysql=mysqli_connect("localhost","2012061516","940710","project") or die("数据库连接失败".mysql_error());
									$mysql->query("set names utf8");
									$sql="select * from meetings ORDER BY beginTime DESC";
									$query=$mysql->query($sql);
									$nums=mysqli_num_rows($query);
									$maxPage = $nums/8-1;
									if ($meetingPage && $maxPage > 0) {
										if ($meetingPage < $maxPage) {
											$prePage = $meetingPage-1;
											$nextPage = $meetingPage+1;
											$nextUrl = 'meetings.php?meetingPage='.$nextPage;
											$preUrl = 'meetings.php?meetingPage='.$prePage;
											echo "<li ><a href='$preUrl'>Previous</a></li>";
											echo "<li id='page'>第".(++$meetingPage)."页</li>";
									...
									elseif (!$meetingPage && $maxPage > 0) {
										$nextUrl = 'meetings.php?meetingPage=1';
