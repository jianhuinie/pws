...
<head>
	...
	<link rel="stylesheet/less" href="../css/comments.less" type="text/less">
	...
	<div id="container">
		<div id="box">
			...
			<div id="nav">
					 ...
					 <a href="comments.php" class="active">会议评述</a>
					...
					<div id="texts">
							 ...
							 <table class="table">
							 	 <thead>
							 	  ...
							 	  <?php
									...
									$mysql->query("set names utf8");
									if ($commentPage > 0) {
										$rangeDown = $commentPage*8;
										//$rangeUp = $rangeDown+8;
										$sql="select * from comments ORDER BY releaseTime DESC LIMIT $rangeDown,8";
									}
									else {
										$sql="select * from comments ORDER BY releaseTime DESC LIMIT 0,8";
									}
									$query=$mysql->query($sql);
							...
							<ul class="pager">
								<?php
									...
									$commentPage = $_GET['commentPage'];
									 $mysql=mysqli_connect("localhost","2012061516","940710","project") or die("数据库连接失败".mysql_error());
									$mysql->query("set names utf8");
									$sql="select * from comments ORDER BY releaseTime DESC";
									$query=$mysql->query($sql);
									$nums=mysqli_num_rows($query);
									$maxPage = $nums/8-1;
									if ($commentPage && $maxPage > 0) {
										if ($commentPage < $maxPage) {
											$prePage = $commentPage-1;
											$nextPage = $commentPage+1;
											$nextUrl = 'comments.php?commentPage='.$nextPage;
											$preUrl = 'comments.php?commentPage='.$prePage;
											echo "<li ><a href='$preUrl'>Previous</a></li>";
											echo "<li id='page'>第".(++$commentPage)."页</li>";
									...
									elseif (!$commentPage && $maxPage > 0) {
										$nextUrl = 'comments.php?commentPage=1';
						...
						<div id="right">
							...
							<?php
								$mysql=mysqli_connect("localhost","2012061516","940710","project") or die("数据库连接失败".mysql_error());
								$mysql->query("set names utf8");
								$sql="select * from comments ORDER BY releaseTime DESC LIMIT 15";
								$query=$mysql->query($sql);
								$count = 0;
								while($row=mysqli_fetch_array($query)) {
									$num = $row['number'];
									echo "<div class='listItem'><a href='commentDetail.php?commentNum=$num'>".(++$count).".".$row['articleName']."</a></div>";
								}
