...
<head>
	...
	<link rel="stylesheet/less" href="../css/searchNews.less" type="text/less">
			...
			<div id="content">
				...
				<div id="content-right"> 
					<ul class="breadcrumb">
						...
						<li>
							<a href="./news.php">会议新闻</a> <span class="divider">/</span>
						</li>
					...
					<div id="texts">
						<div id="left">
							<?php
								...
								$searchNews = $_POST['searchNews'];
								//echo "$searchNews";
								$mysql=mysqli_connect("localhost","2012061516","940710","project") or die("数据库连接失败".mysql_error());
								$mysql->query("set names utf8");
								$sql="select * from news where title like '%$searchNews%' ORDER BY releaseTime DESC";
								$query=$mysql->query($sql);
								$nums=mysqli_num_rows($query);
								//echo "$nums";
								if ($nums > 0) {
									echo "<table class='table'><thead><td>所属学科</td><td>新闻标题</td><td>发布时间</td></thead>"; 
									$searchNewsPage = $_GET['searchNewsPage'];
									if ($searchNewsPage > 0) {
										$rangeDown = $searchNewsPage*8;
										//$rangeUp = $rangeDown+8;
										$sql1="select * from news where title like '%$searchNews%' ORDER BY releaseTime DESC LIMIT $rangeDown,8";
									}
									else {
										$sql1="select * from news where title like '%$searchNews%' ORDER BY releaseTime DESC LIMIT 0,8";
									}
									$query1=$mysql->query($sql1);
									while($row1=mysqli_fetch_array($query1)) {
										$num = $row1['number'];
										echo "<tr><td>".$row1['subject']."</td><td><a href='newsDetail.php?newsNum=$num'>".$row1['title']."</a></td><td>".$row1['releaseTime']."</td></tr>";
									}
									echo "</table>";
									echo "<ul class='pager'>";
									$searchNewsPage = $_GET['searchNewsPage'];
									$maxPage = $nums/8-1;
									if ($searchNewsPage && $maxPage > 0) {
										if ($searchNewsPage < $maxPage) {
											$prePage = $searchNewsPage-1;
											$nextPage = $searchNewsPage+1;
											$nextUrl = 'searchNews.php?searchNewsPage='.$nextPage;
											$preUrl = 'searchNews.php?searchNewsPage='.$prePage;
											echo "<li ><a href='$preUrl'>Previous</a></li>";
											echo "<li id='page'>第".(++$searchNewsPage)."页</li>";
									...
									elseif (!$searchNewsPage && $maxPage > 0) {
										$nextUrl = 'searchNews.php?searchNewsPage=1';
