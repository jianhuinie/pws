...
<head>
	...
	<link rel="stylesheet/less" href="../css/articles.less" type="text/less">
	...
	<div id="container">
		<div id="box">
			...
			<div id="nav">
					 ...
					 <a href="articles.php" class="active">会议论文</a>
			...
			<div id="content">
				...
				<div id="content-right">
					<div id='searchForm'>
						<form method="post">	 
						    <input type="text" placeholder="搜索会议论文" name="searchThesis" id="searchThesis">
						    <input type="submit" value='Search' onclick="if (validateform()) { form.action='./searchThesis.php' }"> 
						</form>
					</div> 
					<ul class="breadcrumb">
						<li>
						...
						<div id="left">
							 <table class="table">
							 	 ...
							 	 <?php
									...
									$mysql->query("set names utf8");
									if ($articlePage > 0) {
										$rangeDown = $articlePage*8;
										//$rangeUp = $rangeDown+8;
										$sql="select * from thesis where agreeVotes > 2 ORDER BY releaseTime DESC LIMIT $rangeDown,8";
									}
									else {
										$sql="select * from thesis where agreeVotes > 2 ORDER BY releaseTime DESC LIMIT 0,8";
									}
									$query=$mysql->query($sql);
									while($row=mysqli_fetch_array($query)) {
										$thesisNum = $row['number'];
										$meetingNum = $row['meetingNum'];
										$sql1 ="select * from meetings where number='$meetingNum'";
										$query1=$mysql->query($sql1);
										$row1=mysqli_fetch_array($query1);
										echo "<tr><td>".$row['username']."</td><td><a href='thesisDetail.php?thesisNum=$thesisNum'>".$row['title']."</a></td>"."<td><a href='meetingDetail.php?meetingNum=$meetingNum'>".$row1['name']."</a></td><td>".$row['releaseTime']."</td></tr>";
									}		 
								?>
							 </table>
							<ul class="pager">
								<?php
									...
									$articlePage = $_GET['articlePage'];
									 $mysql=mysqli_connect("localhost","2012061516","940710","project") or die("数据库连接失败".mysql_error());
									$mysql->query("set names utf8");
									$sql="select * from thesis where agreeVotes > 2 ORDER BY releaseTime DESC";
									$query=$mysql->query($sql);
									$nums=mysqli_num_rows($query);
									$maxPage = $nums/8-1;
									if ($articlePage && $maxPage > 0) {
										if ($articlePage < $maxPage) {
											$prePage = $articlePage-1;
											$nextPage = $articlePage+1;
											$nextUrl = 'articles.php?articlePage='.$nextPage;
											$preUrl = 'articles.php?articlePage='.$prePage;
											echo "<li ><a href='$preUrl'>Previous</a></li>";
											echo "<li id='page'>第".(++$articlePage)."页</li>";
									...
									elseif (!$articlePage && $maxPage > 0) {
										$nextUrl = 'articles.php?articlePage=1';
