...
<head>
	...
	<link rel="stylesheet/less" href="../common/commentDetail.less" type="text/less">
			...
			<div id="content">
				...
				<div id="content-right"> 
					<ul class="breadcrumb">
						...
						<li>
							<a href="comments.php">会议评述</a> <span class="divider">/</span>
						</li> 
						<?php
							$commentNum = $_GET['commentNum'];
							echo "<li class='active'>会议评述$commentNum</li>";
						?>	
					</ul>
					...
					<div id="texts">
						<div id="left">
							<?php
							    ...
							    $mysql=mysqli_connect("localhost","2012061516","940710","project") or die("数据库连接失败".mysql_error());
								$mysql->query("set names utf8");
								$sql="select * from comments where number='$commentNum'";
								$query=$mysql->query($sql);
								$row=mysqli_fetch_array($query);
								...
								$query1=$mysql->query($sql1);
								$row1=mysqli_fetch_array($query1);
								echo "<div id='title'>".$row['articleName']."</div>";	
								echo "<hr>";
								echo "<div class='commentItem'>
								<span>日期：</span>".$row['releaseTime']."</div>";
								echo "<div class='commentItem'>
								<span>稿件来源：</span>".$row['source']."</div>";
								echo "<div class='commentItem'>
								<span>所属会议：</span>".$row1['name']."</div>";
