...
<head>
	...
	<link rel="stylesheet/less" href="../css/notifiers.less" type="text/less">
	...
	<div id="container">
		<div id="box">
			...
			<div id="nav">
					 ...
					 <a href="notifiers.php" class="active">会议通知</a> 
					...
					<div id="texts">
							 ...
							 <table class="table">
							 	 <thead>
							 	  ...
							 	  <?php
							 	 	...
							 	 	$notifierPage = $_GET['notifierPage'];
								    $mysql=mysqli_connect("localhost","2012061516","940710","project") or die("数据库连接失败".mysql_error());
									$mysql->query("set names utf8");
									if ($notifierPage > 0) {
										$rangeDown = $notifierPage*8;
										//$rangeUp = $rangeDown+8;
										$sql="select * from notifiers ORDER BY releaseTime DESC LIMIT $rangeDown,8";
									}
									else {
										$sql="select * from notifiers ORDER BY releaseTime DESC LIMIT 0,8";
									}
									$query=$mysql->query($sql);
									while($row=mysqli_fetch_array($query)) {
										...
										$row1=mysqli_fetch_array($query1);
										//$meeting = $row1['title'];
										echo "<tr><td>"."<a href='meetingDetail.php?meetingNum=$meetingNum'>".$row1['name']."</td><td><a href='notifierDetail.php?notifierNum=$num'>".$row['title']."</a></td><td>".$row['releaseTime']."</td></tr>";
									}		 
								?>
							 </table>
							<ul class="pager">
								<?php
									...
									$notifierPage = $_GET['notifierPage'];
									 $mysql=mysqli_connect("localhost","2012061516","940710","project") or die("数据库连接失败".mysql_error());
									$mysql->query("set names utf8");
									$sql="select * from notifiers ORDER BY releaseTime DESC";
									$query=$mysql->query($sql);
									$nums=mysqli_num_rows($query);
									$maxPage = $nums/8-1;
									if ($notifierPage && $maxPage > 0) {
										if ($notifierPage < $maxPage) {
											$prePage = $notifierPage-1;
											$nextPage = $notifierPage+1;
											$nextUrl = 'notifiers.php?notifierPage='.$nextPage;
											$preUrl = 'notifiers.php?notifierPage='.$prePage;
											echo "<li ><a href='$preUrl'>Previous</a></li>";
											echo "<li id='page'>第".(++$notifierPage)."页</li>";
									...
									elseif (!$notifierPage && $maxPage > 0) {
										$nextUrl = 'notifiers.php?notifierPage=1';
						...
						<div id="right">
							...
							<?php
								$mysql=mysqli_connect("localhost","2012061516","940710","project") or die("数据库连接失败".mysql_error());
								$mysql->query("set names utf8");
								$sql="select * from notifiers ORDER BY releaseTime DESC LIMIT 14";
								$query=$mysql->query($sql);
								$count = 0;
								while($row=mysqli_fetch_array($query)) {
									$num = $row['number'];
									echo "<div class='listItem'><a href='notifierDetail.php?notifierNum=$num'>".(++$count).".".$row['title']."</a></div>";
								}
