...
<head>
	...
	<link rel="stylesheet/less" href="../css/news.less" type="text/less">
	...
	<div id="container">
		<div id="box">
			...
			<div id="nav">
					 ...
					 <a href="news.php" class="active">会议新闻</a> 
			...
			<div id="content">
				...
				<div id="content-right"> 
					<div id='searchForm'>
						<form method="post">	 
						    <input type="text" placeholder="搜索会议新闻" name="searchNews" id="searchNews">
						    <input type="submit" value='Search' onclick="if (validateform()) { form.action='./searchNews.php' }"> 
						</form>
					</div> 
					<ul class="breadcrumb">
						<li>
						...
						<div id="left">
							 <table class="table">
							 	 ...
							 	 <?php
									...
									$mysql->query("set names utf8");
									if ($newsPage > 0) {
										$rangeDown = $newsPage*8;
										//$rangeUp = $rangeDown+8;
										$sql="select * from news ORDER BY releaseTime DESC LIMIT $rangeDown,8";
									}
									else {
										$sql="select * from news ORDER BY releaseTime DESC LIMIT 0,8";
									}
									$query=$mysql->query($sql);
									while($row=mysqli_fetch_array($query)) {
										$num = $row['number'];
										echo "<tr><td>".$row['subject']."</td><td><a href='newsDetail.php?newsNum=$num'>".$row['title']."</a></td><td>".$row['releaseTime']."</td></tr>";
									}		 
								?>
							 </table>
							<ul class="pager">
								<?php
									...
									$maxPage = $nums/8-1;
									if ($newsPage && $maxPage > 0) {
										if ($newsPage < $maxPage) {
											$prePage = $newsPage-1;
											$nextPage = $newsPage+1;
											$nextUrl = 'news.php?newsPage='.$nextPage;
											$preUrl = 'news.php?newsPage='.$prePage;
											echo "<li ><a href='$preUrl'>Previous</a></li>";
											echo "<li id='page'>第".(++$newsPage)."页</li>";
									...
									elseif (!$newsPage && $maxPage > 0) {
										$nextUrl = 'news.php?newsPage=1';
