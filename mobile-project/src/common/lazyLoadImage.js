/**
 * Created by xuzheng on 15/12/28.
 */
    ...
    var createViewportFilter = (function () {
        ...
        function isHidden(element) {
            ...
            if (!len) {
                ...
                var flag = true;
                var lazyID = util_base.getUid(element);
                for (i = 0; i < len; i++) {
                    if (util_base.getUid(sameDom[i]) === lazyID) {
                        flag = false;
                        break;
                    }
                }
    ...
    var fn_autoCut = createLazyFn(function ($element, lazyOptions, options) {
        ...
        parseImageCssSize($element, function (cssSize) {
            if (cssSize.width > 0 && cssSize.height > 0) {
                oss_params['c'] = 1;
                oss_params['e'] = 1;
                oss_params['w'] = cssSize.width;
                oss_params['h'] = cssSize.height;
            }
            lazyOptions.fn_done();
        });
    });
    ...
    var throttle = (function () {
        ...
        return function () {
            ...
            if (delta > check_appear_throttle_time) {
                ...
                timer = window.setTimeout(util_function.bind(viewportFilter.check, null, viewportFilter), delta);
            }
        };
    })();
