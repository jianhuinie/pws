 ...
 *         false: 滑到页面底部，上拉加载
 *     注意事项：
 *     1、element兄弟节点不能有class="has-more"元素
 *     2、dispose的时候会把has-more移除
 * @author hurry
 * @date 2016/12/20
 */
define(function () {
     ...
     * @param  {jQuery} options.element 该元素之后添加has-more元素
    ...
    DropLoad.prototype.init = function (options) {
        ...
        this.loadingDiv = element.next('.has-more');
        if (!this.loadingDiv.length) {
            this.loadingDiv = $(''
                + '<div class="has-more hide">'
                +   '<span class="loading">'
                +       '<i class="t1"></i>'
                +       '<i class="t2"></i>'
                +       '<i class="t3"></i>'
