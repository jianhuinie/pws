...
{{*这个地方放至需不需要广告条，前提是report函数里面有广告*}}
{{$isShowAds = true}}
{{$isNeedScale = true}}
    ...
    <script>
        ...
        gsx_ready(function (page_config) {
            require(['common/analysis'], function (analysis) {
                var pvParam = {
                    ...
                    "params": {
                        "user_role": page_config.user ? page_config.user.type : '',
                ...
                if (!page_config.env.is_app) {
                    analysis.sku({
                        "guid": page_config.log[0],
                        "page_type": "{{$ext_data.page}}",
                        'terminal': 'm',
                        'city_id': page_config.city.id,
                        'sku_id': '{{$sku_id}}'
                    });
                }
                analysis.watchClick({
        ...
        window.ErrTrace = (function (page_name) {
            ...
            function onErrorHandler(msg, fileName, lineNo, columnNo, errorObject) {
                ...
                if (ua.toLowerCase().indexOf('baiduboxapp') != -1) {
                    return;
                }
                var info = [];
                if (fileName) {
...
<body>
{{include file="page/_base/parts_base/domain.tpl"}}
