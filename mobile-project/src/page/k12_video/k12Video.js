...
define(function(require) {
    ...
    function getSingleItem(cCourse) {
        return ['<li>',
            '<a href="' + cCourse.url + '">',
    ...
    function setZSDSingleBlock(blockObj) {
        var courseItems = []
        for (var i = 0; i < blockObj.courses.length; i++) {
            courseItems.push(getSingleItem(blockObj.courses[i]));
        }
        if (zsd2levelDoms[blockObj.id]) {
            $(zsd2levelDoms[blockObj.id]).append(courseItems.join(''));
        } else {
            var goTop = '',
                h1 = '',
                h2 = '';
            if (!page_data.is_redu) {
                goTop = zsd1levelDoms[blockObj.nianji] ? 'go-top' : '';
                h1 = goTop ? '' : '<h2>' + blockObj.nianji + '</h2>';
                h2 = '<h2 class="h2-line2">' + blockObj.name + '</h2>';
                if (!goTop) {
                    zsd1levelDoms[blockObj.nianji] = true;
                }
                var blockItem = ['<div id="block_', blockObj.id, '" class="class-block ', goTop, ' ">',
                    h1, h2,
                    '<ul>',
                    courseItems.join(''),
                    '</ul>',
                ...
                $('.class-content').append($(blockItem));
            } else {
                $('.class-block ul').append($(courseItems.join('')));
            }
    ...
    function getZSDBlock(pageItems) {
        if (page_data.is_redu) {
            setZSDSingleBlock({
                courses: pageItems
            });
            ...
            for (var key in pageItems) {
                setZSDSingleBlock(pageItems[key]);
            }
        }
    ...
    function getNextPage() {
        ...
        function updateMoreDom(hasMore, nextCursor) {
            if (hasMore && nextCursor) {
                ...
                var $noMoreTip = $('<div class="none-button"><div class="none-button-text">没有更多结果了</div></div>');
        ...
        function getNextPageContent() {
            ...
            var data_url = location.pathname.split('/')[2];
            var data = {
                render: 'json',
                page: page
            };
            ...
            $.ajax({
                url: '/video_course/' + data_url + '/',
                data: data,
                success: function(res) {
                    hDom.removeClass('loading');
                    if (res.code == 0) {
                        getZSDBlock(res.data.courses);
                        updateMoreDom(res.data.has_more, +page + 1);
                        lazyLoadImage.init();
                        setTimeout(function(){
                            isLoading = false;
    ...
    function doSearch(searchReason) {
        ...
        location.href = encodeURI(encodeURI(location.origin + '/video_course/zhishi-' + termStrimg));
    ...
    function setShareInfo() {
        var url = require.toUrl("./images/share_img.jpg");
        var options = {
            title: "知识视频库",
            content: "10分钟解决一个必考知识点，提分更轻松！",
            img: url,
            url: location.href
        };
        setShare(options);
