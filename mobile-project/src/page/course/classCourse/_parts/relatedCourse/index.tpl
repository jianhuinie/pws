...
<div class="related-course detail-item">
    <div class="title detail-title">相关课程</div>
    <div class="course-list detail-content">
                ...
                <a href="{{$list.url}}"
                   ...
                   {{/if}}>
                    <div class="item analysis-habo-log" data-habo-type="{{$gsType}}" data-habo-stype="clcourse">
                        ...
                        <div class="right-col">
                            <div class="top-line">
                                <span class="type">{{if $list.type == 1}}1对1{{elseif $list.type == 2}}班课{{elseif
                                    $list.type == 3}}视频课{{/if}}</span>
                            ...
                            <div class="bottom-line">
                                <p class="rc-price{{if empty($list.current_price) || $list.current_price eq 0.0}} rc-price-free{{/if}}">
                                    ...
                                    秒杀价:￥{{$list.limited_discount.price}}
                                        <span class="delete-price{{if empty($list.current_price)}} rc-price-free{{/if}}">{{if !empty($list.current_price)}}￥{{$list.current_price}}{{else}}免费{{/if}}</span>
                                    {{elseif !empty($list.realtime_price)}}
                                    插班价:￥{{$list.realtime_price}}
                                        <span class="delete-price{{if empty($list.current_price)}} rc-price-free{{/if}}">{{if !empty($list.current_price) && $list.current_price neq 0.0}}￥{{$list.current_price}}{{else}}免费{{/if}}</span>
                                    {{else}}
                                    {{if !empty($list.current_price) && $list.current_price neq 0.0}}￥{{$list.current_price}}{{else}}免费{{/if}}
                                    {{if !empty($list.origin_price)}}
                                    <span class="delete-price">￥{{$list.origin_price}}</span>
                                    {{/if}}
                                    {{/if}}
                                </p>
                            </div>
        ...
        {{if $script_data.pageType == 'relatedCourse'}}
            <div class="detail-more"><span class="more-btn related-course-more analysis-habo-log" data-habo-type="{{$gsType}}" data-habo-stype="morecourse">查看更多课程</span></div>
        {{else}}
            {{if $ext_data.is_app}}
