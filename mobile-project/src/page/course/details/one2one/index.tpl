...
{{block name="page"}}
    ...
    {{$page_module = "page/course/details/one2one/index"}}
...
{{block name="data"}}
    ...
    {{$shareInfo = $tpl_data.share_info}}
    {{$shareInfo.url = $shareInfo.share_url}}
    {{$shareInfo.title = $shareInfo.course_name}}
    {{$shareInfo.img = $shareInfo.image}}
    {{$shareInfo.content = $shareInfo.share_content}}
    {{$script_data.shareInfo = Json_encode($shareInfo)}}

    {{$script_data.is_staging = $tpl_data.is_staging}}

{{/block}}
{{block name="style"}}
    <link rel="stylesheet" href="{{$static_origin}}/src/page/course/details/one2one/index.styl"/>
{{/block}}
{{block name="content"}}
    ...
    <div id="main">
        ...
        {{include file="../component/cover/coverOne2One.tpl"}}

        {{*课程简介*}}
        {{$briefIntro.name = $courseInfo.name}}
        {{$briefIntro.courseInfo = "近90天授"}}
        {{$briefIntro.courseInfoDetail = $orgInfo.course_count|cat:"课时"}}
        ...
        {{*底导*}}
        {{include file="../component/courseBottom/one2oneBottom.tpl" templateModel="default"}}
