...
define(function(require, exports) {
    ...
    function createNewPage() {
        ...
        var newPageElement;
        var htmlArray = [];
        var courseCategory = scriptData.courseCategory;
        var courseTitle = scriptData.courseTitle;
        var lessonWay = JSON.parse(scriptData.lessonWay);
        var lessonHours = JSON.parse(scriptData.lessonHours);
        var max = lessonHours.length;
        var model = scriptData.page_model;
        var hours = '';
        htmlArray.push('<div class="buy-content buy-content_' +model + '">');
        htmlArray.push('<div class="choose-content">');
        htmlArray.push('<div class="course-content">');
        if (courseCategory) {
            htmlArray.push('<div class="course-title">' + courseCategory + '-' + courseTitle + '</div>');
        } else {
            htmlArray.push('<div class="course-title">' + courseTitle + '</div>');
        }
        htmlArray.push('<i class="close-page icon-plus-o"></i></div>');
        htmlArray.push('<div class="class-way">');
        htmlArray.push('<div class="text">上课方式</div><ul>');
        //上课方式
        for (var key in lessonWay) {
            if (!!lessonWay[key]) {
                htmlArray.push('<li class="' + key + '" data-way="' + key + '" data-price="' + lessonWay[key] + '" >' + LESSON_WAY_MAP[key] + '</li>');
            }
        }
        htmlArray.push('</ul><span class="way hide"></span><span class="price hide"></span></div>');
        htmlArray.push('<div class="class-hours"><div class="text">课时选择</div><ul>');
        htmlArray.push('<li class="custom" data-discount="0">自定义</li>');
        //课时选择
        for (var i = 0; i < max; i++) {
            hours = lessonHours[i].hours + '小时';

            if (lessonHours[i]['discount'] === '10') {
                hours = hours + '(无折扣)';
            } else {
                hours = hours + '(' + lessonHours[i].discount + '折)';
            }

            htmlArray.push('<li class="discount-' + lessonHours[i]['discount'] + '"data-id="' + lessonHours[i].id + '"data-discount="' + lessonHours[i].discount + '" data-hours="' + lessonHours[i].hours + '">' + hours + '</li>');

        }
        htmlArray.push('</ul></div>');
        htmlArray.push('<div class="buy-count">' + '<div class="text">购买数量</div>');
        htmlArray.push('<div class="choose-count"><span class="min">－<span></span></span><span class="count">1</span><span class="add">＋</span></div></div></div>');
        htmlArray.push('<div class="total-price">');
        htmlArray.push('<div class="total-text"><span class="total">共计：</span><span class="price">---</span></div>');
        htmlArray.push('<div class="sub-button"><div class="btn-text">确定</div></div>');
        htmlArray.push('</div></div>');


        newPageElement = htmlArray.join('');
        var initEvents = false;
        if (!dialog) {
            dialog = new SlideInDialog({ content: newPageElement });
            initEvents = true;
        }
        ...
        var headHeight = $('.head img').height();
        var txt = $('.buy-content').find('.total-price .sub-button .btn-text');
        headHeight = headHeight - 75;

        $('.slide-dialog-mask').css('position', 'fixed');
        $('.buy-content').height(wHeight - headHeight);
        $('.choose-content').height(wHeight - headHeight - 50).css('padding-bottom', '30px');
        if (initEvents) {
            //关闭按钮
            ...
            //选择上课方式
            $('.class-way').on('click', 'li', function() {
                txt.text('确定');
                $(this).addClass('active').siblings().removeClass('active');
                var unitPrice = $(this).attr('data-price');
                var way = $(this).attr('data-way');
                var count = $('.choose-count .count').text();
                var flag = false;
                var price;
                //更新内容
                $('.class-way').css('padding-bottom', '0');
                $('.class-way .way').removeClass('hide').text(LESSON_WAY_MAP[way]);
                $('.class-way .price').removeClass('hide').text('￥' + unitPrice + '/小时');
                $('.class-hours').find('li').each(function() {
                    if ($(this).hasClass('active')) {
                        ...
                        if ($(this).hasClass('custom')) {
                            ...
                            price = (unitPrice * hours * discount / 10).toFixed(2);
                        }
                    }
                });
                if (!flag) {
                    $('.class-hours .custom').addClass('active');
                    price = (unitPrice * count).toFixed(2);
                }
                refreshPrice(price);
            ...
            //选择课时
            $('.class-hours').on('click', 'li', function() {
                txt.text('确定');
                $(this).addClass('active').siblings().removeClass('active');
                var discount = $(this).attr('data-discount');
                var count = $('.choose-count .count').text();
                ...
                if ($(this).hasClass('custom')) {
                    $('.buy-count').show();
                    $('.class-way').find('li').each(function() {
                        if ($(this).hasClass('active')) {
                            uprice = +$(this).attr('data-price');
                            price = (uprice * count).toFixed(2);
                        }
                    });
                } else {
                    var hours = +$(this).attr('data-hours');
                    ...
                    $('.class-way').find('li').each(function() {
                        if ($(this).hasClass('active')) {
                            uprice = +$(this).attr('data-price');
                            price = (uprice * hours * discount / 10).toFixed(2);
                        }
                    });
                }
                refreshPrice(price);
            ...
            //购买数量选择
            var chooseCount = $('.choose-count .count');
            var min = $('.choose-count .min');
            min.on('touchend', function() {
                var value = chooseCount.text();
                var uPrice = +$('.class-way li.active').attr('data-price');
                var price;
                if (value == 1) {
                    return;
                } else {
                    txt.text('确定');
                    if (value == 2) {
                        min.css('background-color', '#fcfdfd');
                    }
                    price = (uPrice * (value - 1)).toFixed(2);
                    chooseCount.text(value - 1);
                }
                refreshPrice(price);

            });
            var add = $('.choose-count .add');
            add.on('touchend', function() {
                var value = +chooseCount.text();
                var uPrice = +$('.class-way li.active').attr('data-price');
                var price;
                chooseCount.text(value + 1);
                min.css('background-color', '#f2f4f5');
                price = (uPrice * (value + 1)).toFixed(2);
                refreshPrice(price);
            ...
            (function() {
                ...
                var data = {};
                data.type = 1;
                data.course_id = scriptData.courseId;
                data.from = scriptData.queryStrFrom;
                var lessonWay;
                $('.sub-button').on('click', function(e) {
                    $('.class-way').find('li').each(function() {
                        if ($(this).hasClass('active')) {
                            flag = true;
                            lessonWay = $(this).attr('data-way');
                            data.lesson_way = lessonWay;
                        }
                    });
                    if (!flag) {
                        ui.remind('请选择上课方式');
                        ...
                        var combo = $('.class-hours li.active');
                        data.combo_id = combo.attr('data-id');
                        if (combo.hasClass('custom')) {
                            data.hours = +$('.choose-count .count').text();
                        }
                        var form = $('#submit');
                        form.find('input').each(function(index, item) {
                         ...
                         if(scriptData.is_staging == 1) {
                             if(checkPrice >= 1000 && checkPrice<=10000){
                                 $('.one2one-staging').find('.item').each(function () {
                                     var there = $(this);
                                     if(there.hasClass('active')) {
                                         checkStagingFlag = true;
                                         periods = there.data('periods');
                                         periodsMoney = there.data('price');
                                     }
                                 });

                                 if(checkStagingFlag === true) {
                                 //请求分期接口
                                     $.ajax({
                                         method: 'post',
                                         url: '/course/fenqiChoice',
                                         data: {
                                             course_number: scriptData.courseNumber,
                                             course_type: 1,
                                             stage_number: periods
                                         },
                                         success: function (response) {
                                             if(response.code == 0) {
                                                 periodsCode = 1; //请求接口成功
                                                 form.submit();
                                             } else {
                                                 periodsCode = 2; //请求接口不成功
                                                 periodsMsg = response.msg;
                                             }
        ...
        window.gsx_ready(function (config) {
            ...
            $('.li-friends').on('click', function() {
                if (isApp && appVersion >= '3.0.2') {
                    ...
                    } else if (appController.isStudentApp()) {
                        doShare('share_pyq', shareInfo);
                    }
                }
            });
