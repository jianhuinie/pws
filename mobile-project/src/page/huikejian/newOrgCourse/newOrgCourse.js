...
define(function (require) {
    ...
    var bindOrgIntroDetail = function () {
        ...
        if (!$(this).hasClass("active")) {
            $(this).find(".slide-down").css({"display": 'block'})
                .next().removeClass("icon-angle-down").addClass("icon-angle-up");
            $(this).addClass("active");
        } else {
            $(this).find(".slide-down").css({"display": "-webkit-box"})
            ...
            if(tell){
                if(appController.getPlatForm() === 'android'){
                    var html = ''
                        ...
                        + '<p style="margin-top:10px;">' + tel_400.replace(',', '转') + '</p>'
                        + '</div>';
                    confirm({
                        content:html,
                        btnText:['取消', '拨打']
    ...
    var qrCodeClick = function () {
        ...
        container
            .on('click', '.qrcode-container .cancel', function () {
                hideQrcode();
            })
    };
    ...
    var DEFAULT_ROWCOUNT = 10;
    // 绑定请求更多课程的点击事件
    var moreCourse = function () {
        var coursePage = container.find('.course-page');
        coursePage
            .on('click', '.more-button', function () {
                var $this = $(this);
                var groupId = $this.attr('data-id');
                var pager = $this.data('pager').split(',');
                var offset = +pager[0];
                var rowCount = +pager[1];
                ajaxCourse($this, groupId, offset + rowCount);
            });

    };

    var ajaxCourse = function (element, groupId, offset) {


        var ulCourse = element.prev();
        element.children('.more-loading').show();
        element.children('.character').hide();
        $.ajax({
            type: "get",
            url: element.data('moreurl'),
            data: {
                org_number: store.get('base_info')['number'],
                offset: offset,
                group_id: groupId,
            ...
            success: function (response) {
                if (response.code == 0) {
                    ulCourse.append(response.data.tpl);
                    lazyLoadImage.init();
                    element.find('.more-loading').hide();
                    var pager = response.data.pager;
                    if (pager.has_more) {
                        element.find('.character').show();
                        element.data('pager', pager.offset + ',' + pager.row_count);
                    }
                    else {
                        element.hide();
                    }
    ...
    var position = function () {
        var groupId = store.get('group_id');
        if (groupId != '') {
            var reCommend = container.find("#" + groupId);
            if (reCommend != undefined && reCommend.length != 0) {
                // 计算该元素离顶部的距离
                var topDistance = $(reCommend).offset().top;
                //document.body.scrollTop = topDistance;
                setTimeout(function () {
                    window.scrollTo(0, topDistance);
                    //var ulRecommend= $(reCommend).find('.recommend-list');
                    //ajaxCourse(ulRecommend,2,groupId);
                }, 100);
    ...
    return {
        init: function () {
            ...
            appointOrg();
            // tianxiaoLog.send(store.get('base_info')['number'], 'courseList');
