<div class="hot-answer">
    {{$hot_answer = $item}}
        <div class="item-nav clearfix">
            ...
            {{if $hot_answer.user_role == 0}}
                <span class="item-nav-name">{{$hot_answer.teacher.display_name|cn_truncate:10}}<span class="item-nav-role">老师</span></span>
                <p class="item-nav-address line-clamp" style="margin-top: 4px;">{{$hot_answer.teacher.short_introduce}}</p>
            {{elseif $hot_answer.user_role == 2}}
                <span class="item-nav-name">{{$hot_answer.student.display_name|cn_truncate:10}}<span class="item-nav-role">学生</span></span>
                <p class="item-nav-address line-clamp" style="margin-top: 4px;">{{$hot_answer.student.short_introduce}}</p>
            {{/if}}
            </span>
        </div>
        ...
        <div class="answer-show">
            ...
            {{/if}}
            {{if isset($hot_answer.pic_url) && $hot_answer.pic_url}}
