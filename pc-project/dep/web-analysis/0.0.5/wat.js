...
(function (global, namespace, interfaceName) {
    ...
    function pgv(options) {
        ...
        // 页面来源traffic_source 注：cookie有效期一天
        var urlParams = parseURL(); // 解析当前页面url
        var referrerParams = parseURL(document.referrer); // 解析来源页面url
        var tomorrow = new Date(new Date().getTime() + 24*60*60*1000);

        if (urlParams.param.traffic_source) { // url参数含traffic_source
            types.push({'traffic_source': urlParams.param.traffic_source});

            WAT.setCookie('traffic_source', urlParams.param.traffic_source, {
                expires: tomorrow
            });
        }
        else if (document.referrer && referrerParams['host'].indexOf('genshuixue') == -1) { // 来自于外站
            types.push({'traffic_source': referrerParams.host});
            if (referrerParams.host != WAT.getCookie('traffic_source')) {
                WAT.setCookie('traffic_source', referrerParams.host, {
                    expires: tomorrow
                });
            }
        }
        else {
            types.push({'traffic_source': WAT.getCookie('traffic_source') || ''});
        }
