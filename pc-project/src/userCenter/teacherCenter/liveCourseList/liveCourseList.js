...
define(function(require, exports) {
    ...
    exports.init = function (data){
        ...
        var cacheCourse = {};
        var cache = localStorage.get('editLiveClassCourese' + userData.id);
        ...
        new Ractive({
            el: '#container',
            template: require('html!./liveCourseList.html'),
            data:{
                    ...
                    buttons: [
                        {
                            ...
                            type: 'primary',
                            action: function () {
                                localStorage.remove('editLiveClassCourese' + userData.id);
                                this.hide();
                                location.reload();
                            }
                        },
            ...
            oncomplete: function () {
                ...
                me.on('stopEnroll', function (event) {
                    var number = $(event.node).closest('.course-item').data('number');
                    confirm({
                        content: '老师，停止招生后，学生将无法继续报名该直播课，仍然要停止招生吗？',
                        title: '温馨提示',
                        width: 335
                    })
                    .done(function () {
                ...
                me.on('closeCourse', function (event) {
                    ...
                    alert({
                        title: '关闭直播课',
                        width: 335,
                        content: '关闭直播课后，我们将取消之前所有学生支付过的订单，该操作不可撤销！',
                        buttons: [
                            {
                                text: '我知道了',
                                type: 'primary',
                ...
                me.on('startEnroll', function (event) {
                    ...
                    confirm({
                        content: '老师，直播课继续招生后，学生将可以继续报名该课程，确认要继续招生吗？',
                        title: '温馨提示',
                        width: 335
                    })
                    .done(function () {
                ...
                //邮寄资料
                me.on('postMaterials', function (event) {
                    var postMaterialUrl = $(event.node).closest('.course-item').data('postmaterial-url');
                    window.open(postMaterialUrl);
                });
