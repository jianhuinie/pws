...
define(function (require, exports) {
                            ...
                            + '<li class="schedule-item" data-sid="${sItem.schedule_id}" data-playbackid="${sItem.playback_id}">'
                                ...
                                + '<div class="schedule-actions">'
                                    + '<!-- if: ${sItem.media_status} == 70 || ${sItem.media_status} == 90 -->'
                                        ...
                                        + '<!-- if: ${sItem.video_item_exist} -->'
                                            + '<span>已添加售卖</span>'
                                        + '<!-- else -->'
                                            + '<span class="to-sale">前往售卖</span>'
                                        + '<!-- /if -->'
                                    + '<!-- else -->'
        ...
        if (start && end) {
            ...
            .done(function (response) {
                if (response.code === 0) {
                    ...
                    var classCourseContain = container.find('#classcourse-list');
                    if (data.course_list) {
                        classCourseContain.html(courseListRender(data));
                        Tooltip.init($('[data-title]'));
                        // 默认打开第一个课程的课节信息
                        classCourseContain.find('.list-item:first').addClass('open');
                    }
                    else {
                        classCourseContain.html('<div class="no-live"><p class="bgImg"></p><p>暂无可用直播回放</p></div>');
                    }
    ...
    exports.init = function () {
        ...
        .on('click', '.to-sale', function (e) { // 前往售卖
            var target = $(e.currentTarget);
            var currSchedule = target.closest('li.schedule-item');
            var scheduleId = currSchedule.data('sid');
