    ...
    getUserInfo = () => {
        ...
        AJAXSERVICE.get(AJAXCONFIG.USER.GET_USER, {}).then(function (res) {
            if (res && res.code === 200) {
                ...
                if (fromPage === 1) {
                    if (!showGiftDialog && !sessionStorage.getItem('haveReceived')) {
    ...
    getLists = (val) => {
        ...
        return AJAXSERVICE.get(AJAXCONFIG.COURSE.GET_COURSE_LIST, params).then(function (res) {
            if (res && res.code === 200) {
                ...
                if (!hasMore && self.dropLoad) {
                    self.dropLoad.dispose();
                    self.dropLoad = null;
                } else if (hasMore && !self.dropLoad) {
                    self.dropLoad = new DropLoad({
                        element: $('.discovery-class-list'),
                        callback: self.getLists
                    });
    ...
    render() {
        ...
        const listComponet = self.state.list.map(function (item) {
            ...
            if (item.courseMode === 1) {
                html = (<SingleClassItem key={item.id + item.courseMode} data={item} />);
            } else {
                html = (<SeriesClassItem discovery key={item.id + item.courseMode} data={item} />);
            }
            return html;
        });
