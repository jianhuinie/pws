...
define(function (require) {
    ...
    observer.addListener(arrDialogs, 'length_changed', function () {
        ...
        if (length) {
            arrDialogs.token = utilDom.insertCssText(cssDialog);
     ...
     *
     * attr:
     *      content {HTMLElement|string|null} 内容
    ...
    function Dialog(options) {
        ...
        this._closeBtnEvt = observer.addDomListener(closeButtonElement, 'click', utilFunction.bind(this.hide, this));

        var popupDiv = new BasePopupDiv({
    ...
    utilBase.inherits(Dialog, MVCObject);
    ...
    Dialog.prototype.title_changed = function () {
        ...
        if (utilBase.isZepto(title) ||
            utilBase.isDom(title) ||
            utilBase.isString(title)) {
    ...
    var utilLang = utilBase;
    publish(Dialog, {
        content: utilLang.union(
            utilLang.isDom,
            utilLang.isZepto,
            utilLang.isString,
            utilLang.isNull
        ),
        title: utilLang.union(
            utilLang.isDom,
            utilLang.isZepto,
            utilLang.isString,
            utilLang.isNull
        ),
        closeButton: utilLang.isBoolean,
        position: utilLang.isString,
        buttons: utilLang.union(
            utilLang.isArray,
            utilLang.isNull
        )
    });
