...
define(function (require) {
    ...
    var lazyLoadImage = require('../lazyLoadImage/index');
    ...
    function ImagePlayer(arrImages, options, titleArray) {
        ...
        if (!utilBase.isNumber(options.index)) {
            options.index = 0;
        }

        options.index = utilMath.clamp(options.index, 0, 1000);

        this.setValues(options);
    }

    utilBase.inherits(ImagePlayer, MVCObject);
    ...
    function Item(container, imageUrl) {
        ...
        $('<img width="100%" data-src="' + imageUrl + '"/>')
            ...
            .on('load', function () {
                ...
                // 如果图片的高度等于屏幕的高度
                if ($(thatImg).height() === $(window).height()) {
                    $(thatImg).css({
                        left: 0,
                        top: 0,
                        width: '100%',
    ...
    utilBase.inherits(Item, MVCObject);
