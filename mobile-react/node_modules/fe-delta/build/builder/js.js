...
const buildDep = function (dependencyMap, output, changeFiles) {
    ...
    if (changeFiles && changeFiles.uptFiles.length) {
        ...
        // var content = fs.readFileSync(path.join(config.releaseDir.js.root, manifestPath), 'utf-8');
        var content = util.cache.get('manifest', output);
        if (content) {
            ...
            console.error('增量构建没有获取到manifest信息');
    ...
    const delFile = function (filePath) {
        ...
        if (!manifest.version[releaseName]) {
            ...
            releaseName = path.join(pluginPath, config.plugin.cssOutputDir, fileName);
            releaseName = releaseName.replace('.styl', '.js');
            delManifestMap(releaseName);
            releaseName = path.join(pluginPath, config.plugin.htmlOutputDir, fileName);
    ...
    util.each(dependencyMap, function (item) {
        ...
        if (util.isJs(filePath) && !(util.isJsMD5InPath(filePath) || util.isExcludeJs(filePath))) {
            addVersion(filePath);
            addDep(filePath);
            if (deps.length) {
    ...
    if (changeFiles && changeFiles.delFiles.length) {
        ...
        // TODO: 是否同时做成定时
        // 有文件删除，前端资源重新加载
        manifest.reloadData = true;
        // return;
    } else if (config.args.total === 0 && !changeFiles) {
    ...
    manifest.dirPath = loaderConfig.dirPath;
    manifest.enableCombo = loaderConfig.enableCombo;
    manifest.prefixKey = config.lsSet.prefixKey;
    manifest.rootPath = config.releaseDir.jsCDNHost;
    ...
    util.each(dependencyMap, function (dps, fileName) {
        if (util.isJs(fileName)) {
            ...
            if (config.args.total === 0) {
                ...
                if (util.isJsMD5InPath(fileName)) {
            ...
            // if (!util.fileFilter.is(fileName, amd.ast)) {
                ...
                if (!(util.isJsMD5InPath(fileName) || util.isExcludeJs(fileName))) {
                    // 重命名，并添加插件，生成AMD插件内容文件
                    renameTask.rename(amd);
                    // JS文件AMD
                    amdTask.amd(amd);
                }
                if (!util.isExcludeJs(fileName)) {
...
exports.build = function (changeFiles) {
    ...
    // 对正向依赖中的AST编辑并发布
    const astBhk = comUtil.benchmark('  js:ast');
    astPublish(dependencyMap, changeFiles);
    astBhk();

    // 插件发布
    const pluginBhk = comUtil.benchmark('  js:css-loader/text:');
    pluginBuild();
    pluginBhk();

    try {
        // 过滤文件处理
        const filterBhk = comUtil.benchmark('  js:manifest:');
        buildDep(dependencyMap, config.manifest, changeFiles);
